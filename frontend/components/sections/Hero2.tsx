...
export default function Hero() {
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-full overflow-hidden bg-[#ffffff] dark:bg-[#0f0f0f]">
      {/* Video Background */}
      <div className="absolute inset-0 z-0">
        ...
        />
        {/* Invert blend — transparent in dark mode = no effect */}
        <div className="absolute inset-0" style={{ backgroundColor: 'var(--hero-blend-color)', mixBlendMode: 'difference', zIndex: 1 }} />
        {/* Desaturate blend — transparent in dark mode = no effect */}
        <div className="absolute inset-0" style={{ backgroundColor: 'var(--hero-blend-color)', mixBlendMode: 'color', zIndex: 2 }} />
        {/* Lightening/darkening overlay — adapts via CSS var */}
        <div className="absolute inset-0 z-10" style={{ backgroundColor: 'var(--hero-overlay)' }} />
        <div className="absolute bottom-0 left-0 right-0 z-20" style={{ height: '180px', background: 'var(--hero-bottom-fade)', backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)', maskImage: 'linear-gradient(to bottom, transparent, black)', WebkitMaskImage: 'linear-gradient(to bottom, transparent, black)' }} />
      </div>
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-0.5 pointer-events-none">
        ...
        >
          <Image
            ...
            width={400}
            height={200}
            className="w-44 sm:w-56 md:w-72 lg:w-80 h-auto object-contain brightness-0 dark:invert"
            priority
          />
        </motion.div>
        <motion.div
          className={`flex items-center gap-3 sm:gap-6 ${isRTL ? 'flex-row-reverse' : ''}`}
        ...
        >
          {tags.map((tag) => (
            <span
              key={tag.key}
              className={`${bodyFont} flex items-center gap-2 text-black/80 dark:text-white/80 text-sm font-bold tracking-wide ${isRTL ? 'flex-row-reverse' : ''}`}
            >
              <span className="w-2 h-2 rounded-full bg-black/80 dark:bg-white/80" />
              {tag.label}
            </span>
          ))}
        </motion.div>
